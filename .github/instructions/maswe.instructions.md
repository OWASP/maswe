...
- **profiles**: MAS Testing Profiles that the weakness is relevant for. Most weaknesses apply to `L1` and `L2`. Resilience weaknesses apply to `R` and Privacy to `P`. Use the current profile values defined in <https://mas.owasp.org/Profiles/>. Additionally, use `EUDIW` for weaknesses covered by the [OWASP MAS EUDI Wallet Profile](https://mas.owasp.org/Profiles/MAS-EUDIW) (EU Digital Identity Wallet); this profile draws on the other profiles and extends beyond their union, so `EUDIW` is added alongside whichever of `L1`/`L2`/`R`/`P` already apply (e.g. `profiles: [L1, L2, EUDIW]`), not used standalone.
