...
- **Local file disclosure**: Through a misconfigured `FileProvider` that exposes internal files to other apps.
...
- **Runtime Checks**: Enforce additional runtime authorization checks by using [`checkCallingPermission` or `checkPermission`](https://developer.android.com/training/permissions/restrict-interactions).
